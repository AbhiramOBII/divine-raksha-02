    ...
    <!-- FOMO Order Notification -->
    <div id="fomo-toast" style="display:none;"
         class="fixed bottom-6 left-6 z-50 max-w-xs bg-white rounded-xl shadow-2xl border border-gray-100 overflow-hidden transition-all duration-500 transform translate-y-4 opacity-0">
        <div class="flex items-start gap-3 p-4">
            <div class="flex-shrink-0 w-10 h-10 bg-green-100 rounded-full flex items-center justify-center">
                <svg class="w-5 h-5 text-green-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"></path></svg>
            </div>
            <div class="flex-1 min-w-0">
                <p class="text-sm text-gray-900 font-medium leading-snug" id="fomo-text"></p>
                <p class="text-xs text-gray-400 mt-1" id="fomo-time"></p>
            </div>
            <button onclick="document.getElementById('fomo-toast').style.display='none'" class="flex-shrink-0 text-gray-300 hover:text-gray-500">
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path></svg>
            </button>
        </div>
        <div class="h-1 bg-gray-100">
            <div id="fomo-progress" class="h-full bg-green-500 transition-all ease-linear" style="width:100%"></div>
        </div>
    </div>
    ...
    <script>
        (function() {
            ...
            const progress = document.getElementById('fomo-progress');
            ...
            function showFomo() {
                if (fomoOrders.length === 0) return;
                const order = fomoOrders[fomoIndex % fomoOrders.length];
                fomoIndex++;

                text.textContent = order.name + ' from ' + order.city + ' just ordered ' + order.product;
                ...
                setTimeout(() => showFomo(), 15000 + Math.random() * 10000);
            }
        })();
    </script>

    <!-- WhatsApp Floating Button -->
    <a href="https://wa.me/919591591759" target="_blank" rel="noopener noreferrer"
       class="fixed bottom-6 right-6 z-50 bg-[#25D366] hover:bg-[#20bd5a] text-white rounded-full w-14 h-14 flex items-center justify-center shadow-lg hover:shadow-xl transition-all duration-300 hover:scale-110"
