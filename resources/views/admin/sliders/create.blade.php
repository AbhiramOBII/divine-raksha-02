        ...
        <form method="POST" action="{{ route('admin.sliders.store') }}" enctype="multipart/form-data"
            ...
            <div x-data="{ preview: null }">
                ...
                <div class="border-2 border-dashed border-gray-300 rounded-xl p-6 text-center hover:border-royal-blue transition-colors"
                    ...
                    <p class="text-xs text-gray-400 mt-2">Recommended: 1920×600px. Max 5MB. JPG, PNG or WebP.</p>
                </div>
                @error('image')
            ...
            <div>
                ...
                <input type="text" name="title" value="{{ old('title') }}"
                       ...
                       placeholder="e.g. Divine Protection for Your Sacred Journey">
                @error('title')
                    <p class="text-sm text-red-600 mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                <textarea name="description" rows="3"
                          class="w-full px-4 py-2.5 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-royal-blue focus:border-royal-blue"
                          placeholder="Subtitle or description text">{{ old('description') }}</textarea>
                @error('description')
                    <p class="text-sm text-red-600 mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="text" name="cta_title" value="{{ old('cta_title') }}"
                           class="w-full px-4 py-2.5 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-royal-blue focus:border-royal-blue"
