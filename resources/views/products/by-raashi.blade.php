    ...
    <div class="bg-gray-50 border-b border-gray-100">
        <div class="container max-w-7xl mx-auto px-4 sm:px-6 py-3">
            <nav class="flex items-center text-sm text-gray-500 flex-wrap">
                ...
                <span class="text-royal-blue font-medium">Shop by Raashi</span>
            </nav>
        </div>
    </div>
                ...
                <aside class="lg:w-64 flex-shrink-0" :class="mobileFilters ? '' : 'hidden lg:block'">
                    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden sticky top-4">
                        <div class="px-5 py-4 border-b border-gray-100">
                            <h3 class="font-semibold text-gray-900 text-base">Shop by Raashi</h3>
                        </div>
                        <div class="p-3 space-y-1">
                            <!-- All -->
                            <a href="{{ route('shop.raashi') }}" class="flex items-center gap-3 px-4 py-3 rounded-xl transition-all duration-200 {{ !$raashi ? 'bg-royal-blue text-white' : 'hover:bg-gray-50 text-gray-700' }}">
                                <span class="w-8 h-8 flex items-center justify-center text-lg">☀️</span>
                                <span class="text-sm font-semibold">All Raashis</span>
                            </a>
                            @foreach($raashis as $name => $data)
                                <a href="{{ route('shop.raashi', $name) }}" class="flex items-center gap-3 px-4 py-3 rounded-xl transition-all duration-200 {{ $raashi === $name ? 'bg-royal-blue text-white' : 'hover:bg-gray-50 text-gray-700' }}">
                                    <span class="w-8 h-8 flex items-center justify-center text-lg">{{ $data['symbol'] }}</span>
                                    <div>
                                        <span class="text-sm font-semibold block">{{ $name }}</span>
                                        <span class="text-xs {{ $raashi === $name ? 'text-white/70' : 'text-gray-400' }}">{{ $data['label'] }}</span>
                                    </div>
                                </a>
                            @endforeach
                        </div>
                ...
                <div class="flex-1 min-w-0">
                    ...
                    @else
                        <div class="text-center py-20">
                            ...
                            <p class="text-gray-500">No products available for this raashi yet.</p>
                        </div>
                    @endif
                </div>
